...
def feature_expectation_from_trajectories(stateToFeatures, trajectories):
    """
...
def expected_svf_from_policy(p_transition, p_initial, terminal, p_action, eps=1e-5):
    ...
    p_transition[list(terminal), :, :] = 0.0
...
def irl(p_transition, stateToFeatures, terminal, trajectories, optim, init, eps=1e-8, eps_esvf=1e-8):
    ...
    while delta > eps:
        ...
        grad = e_features - stateFeatureMatrix.T.dot(e_svf)
...
def local_causal_action_probabilities(p_transition, terminal, reward, discount, eps=1e-5):
    ...
    while delta > eps:
        ...
        q = np.array([reward + discount * p[a].dot(v_old)
                     for a in range(n_actions)]).T
